...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        sucessorScore = successorGameState.getScore()
        minDistanceGhost = float("+inf")
        minDistanceFood = float("+inf")

        # computa distância para o ghost mais próximo.
        for ghostPos in ghostPositions:
            minDistanceGhost = min(minDistanceGhost, util.manhattanDistance(newPos, ghostPos))

        # se a acao selecionada leva muito próximo da colisão com o ghost, pontuação é mínima
        if minDistanceGhost < 2:
            return float("-inf")
        ...
        # computa distância para o food mais próximo.
        for foodPos in newFood.asList():
            minDistanceFood = min(minDistanceFood, util.manhattanDistance(foodPos, newPos))

        sucessorScore += math.log(minDistanceGhost)
        sucessorScore -= minDistanceFood
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        minimax = self.expectimax(gameState,  agentIndex=0, depth=self.depth, action=Directions.STOP)
        return minimax['action']
...
def betterEvaluationFunction(currentGameState):
    ...
    score -= 0.5 * minDistanceFood

    # incentiva o agente a comer pílulas
    score -= 3 * len(newFoodList)
